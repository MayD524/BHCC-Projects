...
class dog : AppCompatActivity() {
    ...
    fun getData(uri: String) : String {
        if (ContextCompat.checkSelfPermission(
            ...
            return try {
                ...
                reader.get("message") as String
            } catch(e: IOException) {

                "Could not tell get dog.. IOException"
            }
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<Button>(R.id.dog_home).setOnClickListener() {
            val output = Intent(this, MainActivity::class.java)
            setResult(RESULT_OK, output)
            finish()
        }

        val img = findViewById<ImageView>(R.id.dog_image)
        val errText = findViewById<TextView>(R.id.dog_errorText)
        Executors.newSingleThreadExecutor().execute {
            Looper.prepare()
            try {
                val nextUrl = getData("https://dog.ceo/api/breeds/image/random")
